    ...
    : IClassFixture<CosmoBaseTestFixture>
{
    ...
    public async Task Test_Direct_Cosmos_SDK()
    {
        ...
        try
        {
            ...
            output.WriteLine($"✅ Direct Cosmos SDK works: {result.Resource.id}");
        }
        catch (Exception ex)
        {
            output.WriteLine($"❌ Even direct Cosmos SDK failed: {ex.Message}");
    ...
    public async Task UpsertAsync_Should_Handle_Multiple_Products()
    {
        ...
        savedProducts.Should()
            .OnlyContain(p => p.CreatedBy == "TestUser", "All products should have correct CreatedBy");
        savedProducts.Should()
            .OnlyContain(p => p.UpdatedBy == "TestUser", "All products should have correct UpdatedBy");
    ...
    public async Task GetAllAsync_Should_Return_Saved_Products()
    {
        ...
        var products =
            SimpleTestDataBuilder
                .CreateSimpleTestProducts(10, "Books"); //TestDataBuilder.CreateTestProducts(5, "electronics");
        foreach (var product in products)
        {
        ...
        retrievedProducts.Should()
            .OnlyContain(p => p.Category == "books", "Should only return books category products");
    ...
    public async Task GetAllAsync_With_Pagination_Should_Support_Offset_And_Limit()
    {
        ...
        var products =
            SimpleTestDataBuilder.CreateSimpleTestProducts(10,
                "pagination"); //TestDataBuilder.CreateTestProducts(5, "electronics");
        foreach (var product in products)
        {
            await writeService.CreateAsync(product);
        }
