...
| 2026-05-24 | #15 — Cross-partition `GetAllAsync()` had no cost warning | Added `LogWarning` in `CosmosRepository<T>.GetAllAsync()` and `CosmosDataReadService<TDto,TDao>.GetAllAsync()` naming the model type and recommending the partition-scoped overload. Added `<remarks>` XML doc on both `ICosmosRepository<T>` and `ICosmosDataReadService<TDto,TDao>` clearly marking the cross-partition fan-out cost and pointing to the safer overload. |
...
### ~~15. Cross-partition fan-out `GetAllAsync()` has no warning~~ ✅ Fixed
